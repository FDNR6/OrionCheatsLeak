...
#include "data.h"
	...
	*DrawList = (DrawListStruct*)&RenderBuff[0];
	...
	Vector2 GetTextSize(const wchar_t* String, RenderToolKit::CharDesc* DescArray, int* DescSize)
	{
		wchar_t WChar;
	...
	bool InitRender(IDXGISwapChain* SwapChain)
	{
		...
		VCall(D3Device, 2); D3D_Device = DeEncPtr(SwapChain, D3Device);

		//get d3d context
		ID3D11DeviceContext* D3Context = nullptr;
		VCall(D3Device, 40, &D3Context);
		if (!D3Context) return false; VCall(D3Context, 2);
	...
	bool NewFrame(IDXGISwapChain* SwapChain, Vector2 ScreenSize)
	{
		//init d3d11
	...
	DrawCmdOut AddDrawCmd(int NumVertex, int NumIndex)
	{
		//lock add cmd
		static bool Lock;
		while (Lock) {};
	...
	__declspec(noinline) void AddPolyFilledAA(const Vector2* Points, int PointsCount, DWORD Color)
	{
		...
		//calculate normals
		Vector2* temp_normals = (Vector2*)_alloca(720);
		for (int i0 = PointsCount - 1, i1 = 0; i1 < PointsCount; i0 = i1++) {
			Vector2 diff = Points[i1] - Points[i0];
		...
		{
			//average normals
			Vector2 dm = (temp_normals[i0] + temp_normals[i1]) * 0.5f;
	...
	__declspec(noinline) void AddPolyLineAA(const Vector2* Points, int PointsCount, DWORD Color, bool Closed, float Thick)
	{
		...
		Vector2* temp_normals = (Vector2*)_alloca(3600);
		Vector2* temp_points = temp_normals + PointsCount;
		...
		else
		{
			...
			for (int i1 = 0; i1 < count; ++i1)
			{
				...
				Vector2 dm_out = dm * (half_inner_thickness + 1.f);
...
namespace Render
{
	...
	//render mgr
	__forceinline bool NewFrame(IDXGISwapChain* SwapChain, Vector2 ScreenSize) {
	...
	//render line
	__forceinline void Line(const Vector2& Start, const Vector2& End, const Color& Color, float Thickness = 1.f) {
		Vector2 Dots[2] = { Start, End }; RenderToolKit::AddPolyLineAA(Dots, 2, Color.DXColor, false, Thickness);
	}
	...
	void FillCircle(const Vector2& Start, const Color& Color, float Radius)
	{
		//gen dots
		Vector2 Dots[90]; int NumDots = 0; //max 90
		for (float i = 0.f; i < 6.28f; i += 0.14f) {
	...
	//render rectangle
	__forceinline void Rectangle(const Vector2& Start, const Vector2& Size, const Color& Color, float Thickness = 1.f)
	{
		//check rounded
		Vector2 Pnts[4] = {
	...
	void RoundedRectangle(const Vector2& Start, const Vector2& Size, const Color& Color, float Radius, float Thickness = 1.f)
	{
		//gen dots
		Vector2 Dots[24];
		Vector2 Add = { 0.f, 0.f };
	...
	void FillRectangle(const Vector2& Start, const Vector2& Size, const Color& Color)
	{
		//add draw cmd
	...
	void FillRoundedRectangle(const Vector2& Start, const Vector2& Size, const Color& Color, float Radius)
	{
		//gen dots
		Vector2 Dots[24];
		Vector2 Add = { 0.f, 0.f };
	...
	//render polygons
	__forceinline void PolyLineArray(Vector2* Dots, int DotsCount, const Color& Color, bool Closed, float Thickness = 1.f) {
		RenderToolKit::AddPolyLineAA(Dots, DotsCount, Color.DXColor, Closed, Thickness);
	}

	__forceinline void PolyFillArray(Vector2* Dots, int DotsCount, const Color& Color) {
		RenderToolKit::AddPolyFilledAA(Dots, DotsCount, Color.DXColor);
	}

	//render text
	Vector2 String(Vector2 Start, const wchar_t* String, bool Center = false, const Color& Color = Render::Color(255, 255, 255))
	{
		...
		if (Center) { Start -= (TextSize / 2.f); /*TextSize /= 2.f;*/ }
